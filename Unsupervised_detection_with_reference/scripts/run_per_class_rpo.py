...
from src.detectors.rpo import RPODetector
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run per-class RPO detection using train/nature as reference "
                    "and held-out real test, plus val/ai as fake test."
    )
    parser.add_argument(
        "--train-features",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="labels",
        help="Label key in NPZ",
    )
    parser.add_argument(
        "--class-key",
        type=str,
        default="class_keys",
        help="Class key field in NPZ",
    )
    parser.add_argument(
        "--reference-label",
        type=int,
        default=0,
        help="Real/nature label, usually 0",
    )
    parser.add_argument(
        "--fake-label",
        type=int,
        default=1,
        help="Fake/ai label, usually 1",
    )
    parser.add_argument(
        "--num-ref",
        type=int,
        default=16,
        help="Number of real samples per class used as reference",
    )
    parser.add_argument(
        "--num-real-test",
        type=int,
        default=4,
        help="Number of held-out real test samples per class",
    )
    parser.add_argument(
        "--num-fake-test",
        type=int,
        default=4,
        help="Number of fake test samples per class from val/ai",
    )
    parser.add_argument(
        "--min-ref-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        default="max",
        choices=["max", "mean"],
        help="Aggregation over projection deviations",
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Apply L2 normalization before fitting/scoring",
    )
    parser.add_argument(
        "--sparsity",
        type=float,
        default=0.1,
        help="Only used when projector-type=sparse",
    )
    parser.add_argument(
        "--eps",
        type=float,
        default=1e-6,
        help="Numerical epsilon for MAD and normalization",
    )
    parser.add_argument(
        "--random-state",
        type=int,
        default=42,
        help="Random seed for class split and projector generation",
    )
    parser.add_argument(
        "--save-dir",
        type=str,
        default="artifacts/per_class_rpo",
        help="Directory to save outputs",
    )
    parser.add_argument(
        "--save-reference-stats",
        action="store_true",
        help="Whether to save fitted RPO reference stats",
    )
...
def main() -> None:
    ...
    train_real_df = train_df[train_df["label"] == args.reference_label].copy()
    ...
    print(f"[Info] n_projections:       {args.n_projections}")
    print(f"[Info] projector_type:      {args.projector_type}")
    ...
    # only save one detector stats as example if requested
    ...
    for class_key in candidate_classes:
        ...
        if args.save_reference_stats and not saved_one_reference:
            ...
            ref_example_path = save_dir / (
                f"reference_example_{args.feature_key}_{args.projector_type}_"
                f"p{args.n_projections}_{args.aggregation}.npz"
            )
    ...
    if used_classes == 0:
        raise RuntimeError(
            "No valid classes found for per-class RPO evaluation. "
    ...
    tag = (
        f"{args.feature_key}_{args.projector_type}_p{args.n_projections}_"
        f"{args.aggregation}_ref{args.num_ref}_rtest{args.num_real_test}_ftest{args.num_fake_test}"
    )
    if args.normalize:
        tag += "_norm"
    ...
    save_scores_npz(
        ...
        n_projections=np.array([args.n_projections], dtype=np.int64),
        projector_type=np.array([args.projector_type], dtype=object),
        aggregation=np.array([args.aggregation], dtype=object),
        normalize=np.array([args.normalize], dtype=bool),
        sparsity=np.array([args.sparsity], dtype=np.float64),
...
# Example usage:
# python scripts/run_per_class_rpo.py \
...
#   --num-fake-test 4 \
#   --n-projections 512 \
#   --projector-type sparse \
#   --aggregation max \
#   --normalize \
#   --save-dir artifacts/per_class_rpo