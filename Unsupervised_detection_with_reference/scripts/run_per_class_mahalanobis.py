...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run per-class Mahalanobis detection using train/nature as reference "
    ...
    parser.add_argument(
        ...
        default="diag",
        choices=["full", "diag"],
        help="Covariance type for Mahalanobis",
    )
    parser.add_argument(
        "--regularization",
        type=float,
        default=1e-3,
        help="Diagonal regularization",
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
    ...
    parser.add_argument(
        "--squared",
        action="store_true",
        help="Use squared Mahalanobis distance",
    )
    parser.add_argument(
        "--min-ref-samples",
        type=int,
        default=None,
        help="Minimum real samples required per class in train. "
             "Default: num_ref + num_real_test",
    )
    parser.add_argument(
        "--random-state",
        type=int,
        default=42,
        help="Random seed for per-class split",
    )
    parser.add_argument(
        "--save-dir",
        type=str,
        default="artifacts/per_class_mahalanobis",
...
def main() -> None:
    ...
    for class_key in candidate_classes:
        ...
        detector.fit(x_ref)
        scores = detector.score(x_test, squared=args.squared)
    ...
    if used_classes == 0:
        raise RuntimeError(
            "No valid classes found for per-class evaluation. "
    ...
    tag = f"{args.feature_key}_{args.covariance_type}_ref{args.num_ref}_rtest{args.num_real_test}_ftest{args.num_fake_test}"
    if args.normalize:
        tag += "_norm"
...
# Example usage:
# python scripts/run_per_class_mahalanobis.py \
...
#   --regularization 1e-3 \
#   --normalize \
#   --save-dir artifacts/per_class_mahalanobis