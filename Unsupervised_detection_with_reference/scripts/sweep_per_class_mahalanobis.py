...
def find_macro_json(save_dir: Path) -> Path:
    ...
    if len(files) > 1:
        print(f"[Warning] Multiple macro json files found in {save_dir}, using latest one.")
    return files[-1]
...
def main() -> None:
    train_features = "artifacts/features/siglip_train.npz"
    val_features = "artifacts/features/siglip_val.npz"
    ...
    output_root = ROOT / "artifacts" / "sweeps" / "per_class_mahalanobis_siglip"
    ...
    for combo in itertools.product(*values):
        ...
        cmd = [
            PYTHON,
            "scripts/run_per_class_mahalanobis.py",
            ...
            "--covariance-type", str(params["covariance_type"]),
        ...
        try:
            ...
            record = {
                ...
                **params,
                ...
                "auroc": payload["macro_metrics"]["auroc"],
                "auprc": payload["macro_metrics"]["auprc"],
                "fpr95": payload["macro_metrics"]["fpr95"],
